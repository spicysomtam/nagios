...
# A Munro: 26 Aug 2015: Perf data not correct. Corrected. Also created a pnp4nagios template to stack the cpu; 8 cores per graph.
# A Munro: 30 Mar 2016: Perf data cpu_avg% not an integer and breaking pnp4nagios graphing.
#################################################################
...
my $o_mcpu=     undef;          # Output multi cpu performance data
...
-m, --multicpu
   Performance data, include cpu data for each processor core.
-t, --timeout=INTEGER
...
foreach my $key ( keys %$resultat) {
   ...
   if ( $key =~ /$proc_load/) {
     ...
     if (defined($o_mcpu)) {$cpu_result .= "cpu$ncpu=$$resultat{$key}%;$o_warn;$o_crit;; "};
#     if (defined($o_mcpu)) {$cpu_result .= "cpu$ncpu=$$resultat{$key}%;;;; "};
     $ncpu++;
   }
}
...
print " < $o_warn% : OK" if ($exit_val eq $ERRORS{"OK"});
if ($ncpu == 1) { $cpu_result="" }
(defined($o_perf)) ?
   print " | cpu_avg=".int($cpu_used)."%;$o_warn;$o_crit;; $cpu_result\n"
#   print " | cpu_avg=$cpu_used%;;;; $cpu_result\n"
 : print "\n";
exit $exit_val;
