...
#
# A Munro 24 Sep 2015: This script is out of data compared to current dd firmware. 
# I have updated it based on the datadomain mib, and now it prints out something meaningful.
# I am not sure measuring space based on the data areas /ddvar, Data and Index
# is meaningful? I think this is a capacity check; eg warn when disk space becomes
# an issue.
#
# A Munro 22 Feb 2016: Allow certain events to be filtered out. Eg:
# A new encryption key has been added to the system
#
...
#               Skip certain issues; add any issues you don't want reporting here:
                if ( $alert =~ /A new encryption key has been added to the system/ ) { next }
...
if ( $variable eq 'DISKUSED' ) {
	...
	if ( ( ! defined $answer ) && ( defined $volume ) ) {
        ...
        if (! defined($critical)) { $critical="" }
	@vol=sort { $a cmp $b } @vol;
	#print "$variable $state - $answer|$perfdata\n";
	#print "$variable $state - ";
	print "FILESYSTEM $state - ";
	for my $i (@vol) { print $i."=".$pct{$i}."% " }
	print "|";
	for my $i (@vol) { print $i."=".$pct{$i}."%;$warning;$critical;0;100 " }
	print "\n";
	exit $ERRORS{$state};
}
